...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    if (is.data.frame(X_test)) {
                        if (!all(c("text","label") %in% names(X_test))) stop("error message")
                        texts <- X_test$text
                    ...
                    pred_res <- predict(texts)
                    preds <- pred_res$pred

                    # confusion matrix
                    labs_all <- sort(unique(c(classes, labels_true, preds)))
                    cm_tbl <- table(factor(labels_true, levels = labs_all), factor(preds, levels = labs_all))
                    cm_df <- as.data.frame(cm_tbl)
                    names(cm_df) <- c("true", "pred", "n")

                    metrics <- tibble(class = labs_all) %>%
                    rowwise() %>%
                    mutate(
                    ...
                    return(list(confusion = cm_tbl, per_class = metrics, summary = summary_metrics, preds = preds))
